...
greens buffer. The terrain attribute buffer moves into the same space,
growing from 72 bytes to the 90 a 60-row hole needs. See
docs/wram_expansion.md for the full plan.
...
from .relocate_attr_buffer import RELOCATE_ATTR_BUFFER_PATCHES
...
from .terrain_bottom_tables import TERRAIN_BOTTOM_TABLE_PATCHES
...
WRAM_EXPANSION_PATCH = CompositePatch(
    ...
    description=(
        ...
        "decompression and the attribute buffer to support holes taller "
        "than 48 rows"
    ),
    patches=[
        *VIEW_OFFSET_TABLE_PATCHES,
